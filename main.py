...
findspark.init('/home/arun/spark-3.1.3-bin-hadoop3.2')
...
def writeToCassandra(writeDF, idDf):
    writeDF.write \
        ...
        .options(keyspace="sf", table="comments") \
        .mode("append") \
        .save()
...
if __name__ == "__main__":
    ...
    commentDF = spark.read \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "localhost:9092") \
        .option("subscribe", "comm") \
    ...
    value_df = commentDF.select(from_json(col("value").cast("string"), schema).alias("values"))

    value_df.show()

    filter_df = value_df.select("values.CommentId", "values.UserID", "values.Comment", "values.VideoID",
                                "values.Datetime")

   # filter_df.show()
    my_udf = spark.udf.register("_nullsafeUDF", lambda str: start(str), StringType())
    Process_df = filter_df.select(col("CommentId"), col("UserId"), col("Comment"), col("VideoID"),
                                  col("Datetime"),
    ...
    KafkaQuery = kafka_target_df.write \
        .option("checkpointLocation", 'tmp/check_point/') \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "localhost:9092") \
        .option("topic", "commentResults") \
    ...
    For_Cass_df = For_Cass_df.withColumn('commentid', F.when(For_Cass_df['commentid'].isNull(),
                                                             "00000000-0000-0000-0000-000000000000").otherwise(col('commentid')))

    CassandraQuery = For_Cass_df.write \
